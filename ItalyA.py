...
st.title('Italy A League 2024/2025 Prediction App')
st.write('Predict the outcome of Italy A League matches using statistical models.')
...
home = st.sidebar.selectbox('Select home team', ['Atalanta', 'Bologna', 'Cagliari', 'Como', 'Empoli', 'Fiorentina', 'Genoa', 'Inter', 'Juventus', 
                                                 'Lazio', 'Lecce', 'Milan', 'Monza', 'Napoli', 'Parma', 'Roma', 'Torino', 'Udinese', 
                                                 'Venezia', 'Verona'])

away = st.sidebar.selectbox('Select away team', ['Atalanta', 'Bologna', 'Cagliari', 'Como', 'Empoli', 'Fiorentina', 'Genoa', 'Inter', 'Juventus', 
                                                 'Lazio', 'Lecce', 'Milan', 'Monza', 'Napoli', 'Parma', 'Roma', 'Torino', 'Udinese', 
                                                 'Venezia', 'Verona'])
...
def load_data():
    try:
        data = pd.read_csv('https://www.football-data.co.uk/mmz4281/2425/I1.csv')
...
def save_to_excel(home_team, away_team, home_goals, away_goals, home_win_prob, draw_prob, away_win_prob, file_name="predictions.xlsx"):
    ...
    new_data = pd.DataFrame({
        'Date': [today_date],
        'League': ['Italy A'],  # Add League column here
        'Home Team': [home_team],
        'Away Team': [away_team],
