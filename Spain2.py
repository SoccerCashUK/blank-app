...
st.title('Spain 2 League 2024/2025 Prediction App')
st.write('Predict the outcome of Spain 2 League matches using statistical models.')
...
home = st.sidebar.selectbox('Select home team', ['Albacete', 'Almeria', 'Burgos', 'Cadiz', 'Cartagena', 'Castellon', 'Cordoba', 'Eibar', 'Elche',
                                                 'Eldense', 'Ferrol', 'Granada', 'Huesca', 'La Coruna', 'Levante', 'Malaga', 'Mirandes', 'Oviedo',
                                                 'Santander', 'Sp Gijon', 'Tenerife', 'Zaragoza'])

away = st.sidebar.selectbox('Select away team', ['Albacete', 'Almeria', 'Burgos', 'Cadiz', 'Cartagena', 'Castellon', 'Cordoba', 'Eibar', 'Elche',
                                                 'Eldense', 'Ferrol', 'Granada', 'Huesca', 'La Coruna', 'Levante', 'Malaga', 'Mirandes', 'Oviedo',
                                                 'Santander', 'Sp Gijon', 'Tenerife', 'Zaragoza'])
...
def load_data():
    try:
        data = pd.read_csv('https://www.football-data.co.uk/mmz4281/2425/SP2.csv')
...
def save_to_excel(home_team, away_team, home_goals, away_goals, home_win_prob, draw_prob, away_win_prob, file_name="predictions.xlsx"):
    ...
    new_data = pd.DataFrame({
        'Date': [today_date],
        'League': ['Spain 2'],  # Add League column here
        'Home Team': [home_team],
        'Away Team': [away_team],
