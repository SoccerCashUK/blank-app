...
st.title('France 1 League 2024/2025 Prediction App')
st.write('Predict the outcome of France 1 League matches using statistical models.')
...
home = st.sidebar.selectbox('Select home team', ['Angers', 'Auxerre', 'Brest', 'Le Havre', 'Lens', 'Lille', 'Lyon', 'Marseille', 'Monaco',
                                                 'Montpellier', 'Nantes', 'Nice', 'Paris SG', 'Reims', 'Rennes', 'St Etienne', 'Strasbourg', 'Toulouse'])

away = st.sidebar.selectbox('Select away team', ['Angers', 'Auxerre', 'Brest', 'Le Havre', 'Lens', 'Lille', 'Lyon', 'Marseille', 'Monaco',
                                                 'Montpellier', 'Nantes', 'Nice', 'Paris SG', 'Reims', 'Rennes', 'St Etienne', 'Strasbourg', 'Toulouse'])
...
def load_data():
    try:
        data = pd.read_csv('https://www.football-data.co.uk/mmz4281/2425/F1.csv')
...
def save_to_excel(home_team, away_team, home_goals, away_goals, home_win_prob, draw_prob, away_win_prob, file_name="predictions.xlsx"):
    ...
    new_data = pd.DataFrame({
        'Date': [today_date],
        'League': ['France 1'],  # Add League column here
        'Home Team': [home_team],
        'Away Team': [away_team],
