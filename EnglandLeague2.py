...
st.title('England League 2 2024/2025 Prediction App')
st.write('Predict the outcome of England League 2 matches using statistical models.')
...
home = st.sidebar.selectbox('Select home team', ['Accrington', 'AFC Wimbledon', 'Barrow', 'Bradford', 'Bromley', 'Carlisle', 
                                                 'Cheltenham', 'Chesterfield', 'Colchester', 'Crewe', 'Doncaster', 
                                                 'Fleetwood Town', 'Gillingham', 'Grimsby', 'Harrogate', 
                                                 'Milton Keynes Dons', 'Morecambe', 'Newport County', 
                                                 'Notts County', 'Port Vale', 'Salford', 'Swindon', 
                                                 'Tranmere', 'Walsall'])

away = st.sidebar.selectbox('Select away team', ['Accrington', 'AFC Wimbledon', 'Barrow', 'Bradford', 'Bromley', 'Carlisle', 
                                                 'Cheltenham', 'Chesterfield', 'Colchester', 'Crewe', 'Doncaster', 
                                                 'Fleetwood Town', 'Gillingham', 'Grimsby', 'Harrogate', 
                                                 'Milton Keynes Dons', 'Morecambe', 'Newport County', 
                                                 'Notts County', 'Port Vale', 'Salford', 'Swindon', 
...
def load_data():
    try:
        data = pd.read_csv('https://www.football-data.co.uk/mmz4281/2425/E3.csv')
...
def save_to_excel(home_team, away_team, home_goals, away_goals, home_win_prob, draw_prob, away_win_prob, file_name="predictions.xlsx"):
    ...
    new_data = pd.DataFrame({
        'Date': [today_date],
        'League': ['England League 2'],  # Add League column here
        'Home Team': [home_team],
        'Away Team': [away_team],
