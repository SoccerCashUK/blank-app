...
st.title('England League 1 2024/2025 Prediction App')
st.write('Predict the outcome of England League 1 matches using statistical models.')
...
home = st.sidebar.selectbox('Select home team', ['Barnsley', 'Birmingham', 'Blackpool', 'Bolton', 'Bristol Rvs', 'Burton', 
                                                 'Cambridge', 'Charlton', 'Crawley Town', 'Exeter', 'Huddersfield', 
                                                 'Leyton Orient', 'Lincoln', 'Mansfield', 'Northampton', 'Peterboro', 
                                                 'Reading', 'Rotherham', 'Shrewsbury', 'Stevenage', 'Stockport', 
                                                 'Wigan', 'Wrexham', 'Wycombe'])

away = st.sidebar.selectbox('Select away team', ['Barnsley', 'Birmingham', 'Blackpool', 'Bolton', 'Bristol Rvs', 'Burton', 
                                                 'Cambridge', 'Charlton', 'Crawley Town', 'Exeter', 'Huddersfield', 
                                                 'Leyton Orient', 'Lincoln', 'Mansfield', 'Northampton', 'Peterboro', 
                                                 'Reading', 'Rotherham', 'Shrewsbury', 'Stevenage', 'Stockport', 
                                                 'Wigan', 'Wrexham', 'Wycombe'])
...
def load_data():
    try:
        data = pd.read_csv('https://www.football-data.co.uk/mmz4281/2425/E2.csv')
...
def save_to_excel(home_team, away_team, home_goals, away_goals, home_win_prob, draw_prob, away_win_prob, file_name="predictions.xlsx"):
    ...
    new_data = pd.DataFrame({
        'Date': [today_date],
        'League': ['England League 1'],  # Add League column here
        'Home Team': [home_team],
        'Away Team': [away_team],
