...
st.title('Netherlands Eredivisie League 2024/2025 Prediction App')
st.write('Predict the outcome of Netherlands Eredivisie League matches using statistical models.')
...
home = st.sidebar.selectbox('Select home team', ['Ajax', 'Almere City', 'AZ Alkmaar', 'Feyenoord', 
                                                 'For Sittard', 'Go Ahead Eagles', 'Groningen', 'Heerenveen', 
                                                 'Heracles', 'NAC Breda', 'Nijmegen', 'PSV Eindhoven', 
                                                 'Sparta Rotterdam', 'Twente', 'Utrecht', 'Waalwijk', 
                                                 'Willem II', 'Zwolle'])

away = st.sidebar.selectbox('Select away team', ['Ajax', 'Almere City', 'AZ Alkmaar', 'Feyenoord', 
                                                 'For Sittard', 'Go Ahead Eagles', 'Groningen', 'Heerenveen', 
                                                 'Heracles', 'NAC Breda', 'Nijmegen', 'PSV Eindhoven', 
                                                 'Sparta Rotterdam', 'Twente', 'Utrecht', 'Waalwijk', 
                                                 'Willem II', 'Zwolle'])
...
def load_data():
    try:
        data = pd.read_csv('https://www.football-data.co.uk/mmz4281/2425/N1.csv')
...
def save_to_excel(home_team, away_team, home_goals, away_goals, home_win_prob, draw_prob, away_win_prob, file_name="predictions.xlsx"):
    ...
    new_data = pd.DataFrame({
        'Date': [today_date],
        'League': ['etherlands Eredivisie'],  # Add League column here
        'Home Team': [home_team],
        'Away Team': [away_team],
