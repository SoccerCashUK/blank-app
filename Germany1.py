...
st.title('Germany 1 League 2024/2025 Prediction App')
st.write('Predict the outcome of Germany 1 League matches using statistical models.')
...
home = st.sidebar.selectbox('Select home team', ['Augsburg', 'Bayern Munich', 'Bochum', 'Dortmund', 'Ein Frankfurt', 'Freiburg', 'Heidenheim', 'Hoffenheim',
                                                 'Holstein Kiel', 'Leverkusen', 'Mainz', "M'gladbach", 'RB Leipzig', 'St Pauli', 'Stuttgart', 'Union Berlin',
                                                 'Werder Bremen', 'Wolfsburg'])

away = st.sidebar.selectbox('Select away team', ['Augsburg', 'Bayern Munich', 'Bochum', 'Dortmund', 'Ein Frankfurt', 'Freiburg', 'Heidenheim', 'Hoffenheim',
                                                 'Holstein Kiel', 'Leverkusen', 'Mainz', "M'gladbach", 'RB Leipzig', 'St Pauli', 'Stuttgart', 'Union Berlin',
                                                 'Werder Bremen', 'Wolfsburg'])
...
def load_data():
    try:
        data = pd.read_csv('https://www.football-data.co.uk/mmz4281/2425/D1.csv')
...
def save_to_excel(home_team, away_team, home_goals, away_goals, home_win_prob, draw_prob, away_win_prob, file_name="predictions.xlsx"):
    ...
    new_data = pd.DataFrame({
        'Date': [today_date],
        'League': ['Germany 1'],  # Add League column here
        'Home Team': [home_team],
        'Away Team': [away_team],
