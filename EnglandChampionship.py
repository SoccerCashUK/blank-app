...
st.title('England Championship 2024/2025 Prediction App')
st.write('Predict the outcome of England Championship matches using statistical models.')
...
home = st.sidebar.selectbox('Select home team', ['Blackburn', 'Bristol City', 'Burnley', 'Cardiff', 'Coventry', 'Derby', 
                                                 'Hull', 'Leeds', 'Luton', 'Middlesbrough', 'Millwall', 'Norwich', 
                                                 'Oxford', 'Plymouth', 'Portsmouth', 'Preston', 'QPR', 
                                                 'Sheffield United', 'Sheffield Weds', 'Stoke', 'Sunderland', 
                                                 'Swansea', 'Watford', 'West Brom'])

away = st.sidebar.selectbox('Select away team', ['Blackburn', 'Bristol City', 'Burnley', 'Cardiff', 'Coventry', 'Derby', 
                                                 'Hull', 'Leeds', 'Luton', 'Middlesbrough', 'Millwall', 'Norwich', 
                                                 'Oxford', 'Plymouth', 'Portsmouth', 'Preston', 'QPR', 
                                                 'Sheffield United', 'Sheffield Weds', 'Stoke', 'Sunderland', 
                                                 'Swansea', 'Watford', 'West Brom'])
...
def load_data():
    try:
        data = pd.read_csv('https://www.football-data.co.uk/mmz4281/2425/E1.csv')
...
def save_to_excel(home_team, away_team, home_goals, away_goals, home_win_prob, draw_prob, away_win_prob, file_name="predictions.xlsx"):
    ...
    new_data = pd.DataFrame({
        'Date': [today_date],
        'League': ['England Championship'],  # Add League column here
        'Home Team': [home_team],
        'Away Team': [away_team],
