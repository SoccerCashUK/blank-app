...
st.title('France 2 League 2024/2025 Prediction App')
st.write('Predict the outcome of France 2 League matches using statistical models.')

# Sidebar elements for selecting teams
home = st.sidebar.selectbox('Select home team', ['Ajaccio', 'Amiens', 'Annecy', 'Bastia', 'Caen', 'Clermont', 'Dunkerque', 'Grenoble',
                                                 'Guingamp', 'Laval', 'Lorient', 'Martigues', 'Metz', 'Paris FC', 'Pau FC', 'Red Star', 'Rodez', 'Troyes'])

away = st.sidebar.selectbox('Select away team', ['Ajaccio', 'Amiens', 'Annecy', 'Bastia', 'Caen', 'Clermont', 'Dunkerque', 'Grenoble',
                                                 'Guingamp', 'Laval', 'Lorient', 'Martigues', 'Metz', 'Paris FC', 'Pau FC', 'Red Star', 'Rodez', 'Troyes'])
...
def load_data():
    try:
        data = pd.read_csv('https://www.football-data.co.uk/mmz4281/2425/F2.csv')
...
def save_to_excel(home_team, away_team, home_goals, away_goals, home_win_prob, draw_prob, away_win_prob, file_name="predictions.xlsx"):
    ...
    new_data = pd.DataFrame({
        'Date': [today_date],
        'League': ['France 2'],  # Add League column here
        'Home Team': [home_team],
        'Away Team': [away_team],
